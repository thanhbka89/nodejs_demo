...
export function listenToProductList({ commit }) {
	return ref.child('products').on('value', (products) => {
		console.log('[firebase.products]', products.val())
		commit('UPDATE_PRODUCT_LIST', products.val())
	})
}

export async function getProductList({ commit }) {
	return ProductService.find()
		.then((response) => commit('UPDATE_PRODUCT_LIST_VLS', response.data.data.docs))
		.catch((e) => console.log(e))
}

export function getShoppingCart({ commit }, { uid, currentCart }) {
	console.log('[firebase.getCart]', { uid, currentCart })
	if (uid) {
		return ref
			.child('cart/' + uid)
			.once('value')
			.then((cart) => {
				console.log('[firebase.cart]', cart.val())
