...
import { queryKiemKe, queryInventory, queryTransDetail, queryItem } from './mmService'
...
async function runTonCuoiKy({date = new Date}) {
  let page = 1, limit = 50000
  let ton_dau_ky = [], nhap_trong_ky = [], xuat_trong_ky = [], services = [], data = []
  ...
  let lastPreviousPeriod = getPreviousMonth({ date: lastDay, format: 'MM/YYYY' })
  ...
  // get NhapTrongKy
  let groups = Object.values(res3.data.reduce((r, o) => {
    let date = o.code
  ...
  // get XuatTrongKy
  groups = Object.values(res4.data.reduce((r, o) => {
  ...
  services.forEach((element, idx) => {
    ...
    // Ton kho cuoi ky
    item.sl_tinhtoan = (parseInt(item.dauky_sl) || 0) + (parseInt(item.receive_trongky_sl) || 0) - (parseInt(item.issue_trongky_sl) || 0)
