...
class ThuChi {
    ...
    static getCondition(input) {
        ...
        const {code, type, status, from, to} = input || {}
        if (code) {
			$where = $where.concat(` ${check ? 'AND' : ''} code = '${code}'`)
			check ++
        }
        if (type) {
			$where = $where.concat(` ${check ? 'AND' : ''} type = ${type}`)
			check ++
        }
        if (status) {
    ...
    static async paginate({page = 1, limit = 5, code, type, status, from, to}) {
        let start = 0
        ...
        const condition = this.getCondition({code, type, status, from, to})
